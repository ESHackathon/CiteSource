...
# add citesource columns where they do not exist 
...
match_citations <- function(formatted_citations){
  ...
  pairs <- pairs  %>%
    ...
    mutate(cite_string1 =formatted_citations$cite_string[id1]) %>%
    mutate(cite_string2 =formatted_citations$cite_string[id2])
...
merge_metadata <- function(raw_citations_with_id, matched_pairs_with_ids){
  ...
  citations_with_dup_id_merged <- all_metadata_with_duplicate_id %>%
    ...
    mutate(across(c(everything(), -cite_label, -cite_source, -cite_string, -record_id), gsub, pattern = ";;;.*", replacement = "")) %>%
    mutate(across(cite_label, gsub, pattern = ";;;", replacement = ", ")) %>%
    mutate(across(cite_source, gsub, pattern = ";;;", replacement = ", ")) %>%
    mutate(across(cite_string, gsub, pattern = ";;;", replacement = ", ")) %>%
