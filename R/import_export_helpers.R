...
write_refs <- function(
  ...
  export <- switch(format,
    ...
    "ris" = {
      write_ris(x, tag_naming = tag_naming)
    }
  )
...
detect_parser <- function(x) {
  ...
  if (any(proportions > 0.2)) {
    result <- switch(c("comma", "tab", "bibtex", "ris")[which.max(proportions)],
      "comma" = "parse_csv",
      "tab" = "parse_tsv",
      "bibtex" = "parse_bibtex",
      "ris" = {
...
#' @rdname detect_
detect_lookup <- function(tags # a vector of strings representing ris tags
) {
...
merge_columns <- function(
  ...
  result_list <- lapply(x, function(a, cn) {
    ...
    if (any(missing_names)) {
      ...
      result <- data.frame(
        c(a, sapply(new_names, function(b) {
          NA
        })),
        stringsAsFactors = FALSE
      )
...
clean_colnames <- function(x # colnames
) {
  if (inherits(x, "data.frame")) {
    x <- colnames(x)
  }
