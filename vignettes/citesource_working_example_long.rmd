...
### Report contributions of different sources in table

You can create a table that reports on the contribution of each source. To keep it simple, this can be restricted to a specific number (`top_n`) of sources.

```{r}
citation_summary_table(unique_citations, screening_label = "Final", top_n = 5)
```

Such summaries can also be created for search strings, if a different grouping is to be explored.

```{r}
citation_summary_table(unique_citations, comparison_type = "strings", screening_label = "Final")
```
