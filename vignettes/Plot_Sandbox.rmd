...
dedup_results <- dedup_citations(citations)
#> formatting data...
...
### NEW TABLE Experimentation (related to issue #119 & #120)
```{r}
#Create a table to show the difference between the initial number of citations uploaded for each source, and the number D-SUR (Database-Specific Unique Records).

#initial_count is correct, but there is something funny going on with the count_D_SUR and the N_Unique_count. It may be something to do with the weird repeating that we see in unique_citations where the record_ids repeat at times. for an example of this see 1728 and 10104. These two records are the reason we see 52 in D-SUR for Econlit rather than 50, which would be correct.
...
count_databases <- function(df, db_colname) {
  ...
  colnames(db_counts) <- c("single_db", "count")
...
n_unique_citations_count <- n_unique %>%
  ...
  dplyr::summarise(N_Unique_Count = sum(unique)) %>%
  dplyr::filter(cite_source != "") %>%
  dplyr::arrange(cite_source) %>%
  dplyr::rename(single_db = cite_source)

# Combine the counts into one table
citation_counts <- dplyr::left_join(initial_citations_count, d_sur_count, by = "single_db", suffix = c("_Initial", "_D_SUR")) %>%
  dplyr::left_join(n_unique_citations_count, by = "single_db", suffix = c("", "_N_Unique"))
