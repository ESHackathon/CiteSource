...
### Original function to show internal deduplication
```{r}
...
###Calcultations for Updated Summary Table
```{r}
library(gt)
library(gtExtras)
# Function to count occurrences of each database
...
```
### Calculations for New precision/sensitivity table
```{r}
library(dplyr)
library(tidyr)
library(stringr)
...
### Unique records count from sources
```{r}
...
```
###NEW count functions and tables
```{r}
