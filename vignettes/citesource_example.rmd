...
vignette: >
  %\VignetteIndexEntry{CiteSource - Example workflow}
...
# for each unique citation, which sources, labels and strings were present
complete_comparison <- compare_sources(unique_citations)
```

## Step 4a: review a summary table

```{r}
citation_summary_table(unique_citations, screening_label = c("screen", "final"))
```
## step 4b: visualise comparisons
...
## Step 4c: review individual records

This will usually be impractical for all records, but it might be worth showing/reporting where final records came from.

```{r}
unique_citations %>% filter(stringr::str_detect(cite_label, "final")) %>% 
  record_level_table(return = "DT")
```

## Step 5: export deduplicated and annotated citations

For further processing, you might want to export the deduplicated and annotated citations.

```{r}
export_bib(unique_citations, "citations.bib")
export_ris(unique_citations, "citations.ris")
export_csv(unique_citations, "citations.csv", separate = "cite_source")

# To continue, data could be reimported
# unique_citations2 <- reimport_ris("citations.ris")
# or as csv
# export_csv(unique_citations, "citations.csv")
